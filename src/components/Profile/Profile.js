...
export default function Profile({ profileData }) {
      ...
      if (profileData && profileData.id) {
        try {
          const data = await callAPI(
            `/user/${profileData.id}/get-avatar?user_id=${profileData.id}`,
          );
          setAvatarUrl(
            data.avatar_url !== 'no avatar' ? data.avatar_url : null,
          );
        } catch (error) {
  ...
  useEffect(() => {
    const fetchFollowData = async () => {
      if (profileData && profileData.id) {
        try {
          const followersData = await callAPI(
            `/user/${profileData.id}/followers`,
          );
          const followingData = await callAPI(
            `/user/${profileData.id}/following`,
          );
      ...
      <div className={styles.profile__details}>
        <div className={styles.profile__details_grid}>
          <p className={styles.profile__detail}>
            <strong>Self assessment level:</strong>{' '}
            {profileData.info.self_assessment_lvl || 'Not specified'}
          </p>
          <p className={styles.profile__detail}>
            <strong>Purpose:</strong>{' '}
            {profileData.info.purpose || 'Not specified'}
          </p>
          <p className={styles.profile__detail}>
          ...
          <p className={styles.profile__detail}>
            ...
            {profileData.info.hours_per_week || '0'}
          </p>
        </div>
      </div>
      {isModalOpen && (
